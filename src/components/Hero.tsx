...
export function Hero() {
  ...
  return (
    <section id="home" className="relative isolate pt-14 lg:pt-0 min-h-screen flex items-center overflow-hidden bg-background">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-accent/10 rounded-full blur-[120px] opacity-40 pointer-events-none"></div>

      <div className="mx-auto max-w-7xl px-6 py-20 sm:py-28 lg:flex lg:items-center lg:gap-x-10 lg:px-8 lg:py-36">
        {/* Left Content */}
        <div className="mx-auto max-w-2xl lg:mx-0 lg:flex-auto">
           <h1 className="max-w-lg text-4xl font-bold tracking-tight text-primary sm:text-6xl text-balance font-oswald">
             <Trans 
               i18nKey="hero.title" 
               components={{ 
                 1: <span className="text-accent" />, 
                 2: <span className="text-text/60" /> 
               }} 
             />
           </h1>
           
           <p className="mt-5 text-lg leading-8 text-text/70 max-w-xl">
             {t('hero.description')}
           </p>
        ...
        <div className="mt-16 sm:mt-24 lg:mt-0 lg:flex-shrink-0 lg:flex-grow w-full max-w-lg mx-auto lg:mx-0">
          ...
          >
            {/* Terminal Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-muted/30 bg-primary/70">
              ...
              <div className="text-xs text-background/60 font-mono animate-line" style={{ animationDelay: '0.2s' }}>{t('hero.terminal.header')}</div>
            </div>
            
            {/* Terminal Content */}
            <div className="p-6 font-mono text-sm text-background/80 space-y-4 min-h-[300px]">
              <div className="flex animate-line" style={{ animationDelay: '0.4s' }}>
                <span className="text-accent mr-2">➜</span>
                <span className="text-background/80 mr-2">~</span>
                <span className="text-background">{t('hero.terminal.command')}</span>
              </div>
              ...
              <div className="pt-4 border-t border-muted/20 animate-line" style={{ animationDelay: '3.6s' }}>
                <div className="text-background/70 mb-2 animate-line flex items-baseline font-mono" style={{ animationDelay: '4.0s' }}>
                  <span className="inline-block w-3 text-right mr-2 text-background/60 select-none">&gt;</span>
                  <Typewriter delay="4.0s" durationSec={1.5}><Trans i18nKey="hero.terminal.strategyCompiled" components={[<span key="0" className="text-background" />]} /></Typewriter>
                </div>
                <div className="text-background/70 mb-2 animate-line flex items-baseline font-mono" style={{ animationDelay: '5.7s' }}>
                  <span className="inline-block w-3 text-right mr-2 text-background/60 select-none">&gt;</span>
                  <Typewriter delay="5.7s" durationSec={1.5}><Trans i18nKey="hero.terminal.growthVector" components={[<span key="0" className="text-background" />]} /></Typewriter>
                </div>
                <div className="text-background/70 mb-2 animate-line flex items-baseline font-mono" style={{ animationDelay: '7.4s' }}>
                  <span className="inline-block w-3 text-right mr-2 text-background/60 select-none">&gt;</span>
